...
namespace DepthAI.Core
{
    public class OAKDevice 
    {
        ...
        public List<System.Drawing.Image> textures;
        ...
        public void Start()
        {
            ...
            textures = new List<System.Drawing.Image>(textureNames.Count);
            for (int i = 0; i < textureNames.Count; i++)
            {
                var texture = new System.Drawing.Bitmap(2,2);
                textures.Add(texture);
            }

            // pipeline is mandatory
            if (pipelines.Count == 0) Debug.WriteLine("No pipeline defined.");
            else
            {
                if (recordResults && pathToRecord == "") Debug.WriteLine("No path to save recording.");
                if (replayResults && pathToReplay == "") Debug.WriteLine("No path to read recording.");
        ...
        */
        ...
        public void Update()
        {
            ...
            _elapsedTime += sw.ElapsedMilliseconds;//.deltaTime;
            sw.Restart();
            if (!(_elapsedTime > 1.0f / replayFPS)) return;
            
            _elapsedTime -= 1.0f / replayFPS;

            var finalPath = PathHelper.AssemblyDirectory + "/Recorded/"+ pathToReplay;
            ...
            for (int i = 0; i < textureNames.Count; i++)
            {
                ...
                textures[i] = System.Drawing.Image.FromStream(new MemoryStream(bytes));
            }
         ...
         */
        public void Record(string json, List<System.Drawing.Image> textures, List<string> textureNames)
        {
            var finalPath = PathHelper.AssemblyDirectory + "/Recorded/" + pathToRecord;
            if (!Directory.Exists(finalPath)) Directory.CreateDirectory(finalPath);
            finalPath+="/result_";
         ...
         */
        public void Record(string json, System.Drawing.Image tex)
        {
            var finalPath = PathHelper.AssemblyDirectory + "/Recorded/" + pathToRecord;
            if (!Directory.Exists(finalPath)) Directory.CreateDirectory(finalPath);
            finalPath += "/result_" + _frame;
